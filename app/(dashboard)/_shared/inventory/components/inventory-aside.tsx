...
interface InventoryAsideProps {
  basePath: string; // e.g. "/admin/inventory" or "/salesperson/inventory"
}

export function InventoryAside({ basePath }: InventoryAsideProps) {
  ...
  // e.g. /admin/inventory/[id]  -> segments[2] = id
  //      /salesperson/inventory/[id] -> segments[2] = id
  ...
  const handleSelectCard = (card: Card) => {
    ...
    router.push(`${basePath}/${card._id}?type=${card.type}`);
  };

  const handleStartRegister = () => {
    setShowListMobile(false);
    router.push(`${basePath}/register?type=bale`);
  };
